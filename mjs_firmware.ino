...
// Value in Ohms (nominally 100k, but measured 98.2 for Lux1 and 98.3 for Lux2)
float const R12 = 98250.0;
// Value in Ohms (nominally 3.3K, but measured 3.267 for Lux1 and 3.275 for Lux2)
float const R11 = 3270.0;
...
luxmeter luxmeters[] = {
  // Original lux meter plug
  {
    .ADC_PIN = A2,
    .CONTROL_PIN = 5,
    .FACTOR = 1,
  },
  // Soil moister sensor plug with added components
  {
    .ADC_PIN = A1,
    .CONTROL_PIN = 6,
    // The constants used in the calculation were determined based on
    // the sensor used for lux1. For lux2, the enclosure has more
    // attenuation, so this roughly corrects the measured lux value
    // (this was not properly calibrated).
    .FACTOR = 3,
  },
};
...
uint32_t const UPDATE_INTERVAL = 60000;
...
void setup() {
  ...
  for (uint8_t i = 0; i < sizeof(luxmeters)/sizeof(*luxmeters); ++i)
    pinMode(luxmeters[i].CONTROL_PIN, INPUT);
...
#ifdef WITH_LUX
    Serial.print(F("Lux1: "));
    Serial.println(luxmeters[0].lux);
    Serial.print(F("Lux2: "));
    Serial.println(luxmeters[1].lux);
#endif // WITH_LUX
...
// Do not append normal lux value
  ...
  // field.
  for (uint8_t i = 0; i < sizeof(luxmeters)/sizeof(*luxmeters); ++i) {
    // Send unscaled value (so 2 extra bits), since extra data is not
    // processed in any way, so this makes it easier to process
    packet.append(18-1, EXTRA_SIZE_BITS);
    packet.append(luxmeters[i].lux, 18);

    // Send raw 10-bit ADC value
    packet.append(10-1, EXTRA_SIZE_BITS);
    packet.append(luxmeters[i].adc_value, 10);
...
#ifdef WITH_LUX
void readLux(uint8_t idx)
{
  uint32_t result = 0;
  uint8_t range = 0;
  luxmeter& lux = luxmeters[idx];

  // Set the Reference Resistor to just R12
  pinMode(lux.CONTROL_PIN, INPUT);
  ...
  analogRead(lux.ADC_PIN);
  uint16_t raw_adc = analogRead(lux.ADC_PIN);
  // Check if read_low has an overflow
  if (raw_adc < 1000)
  {
    result = uint32_t(lx_conv_low * reference_voltage_internal * raw_adc * lux.FACTOR);
    range = 1;
  } else {
    // Set the Reference Resistor to R11 parallel with R12 for more range
    pinMode(lux.CONTROL_PIN, OUTPUT);
    digitalWrite(lux.CONTROL_PIN, LOW);
    ...
    // through R11+R12, which has an RC time of 100nF x 3k = 300μs. To be
    // sure, we wait for 3ms.
    delay(3);
    raw_adc = analogRead(lux.ADC_PIN);
    // Check if read_high has an overflow
    if (raw_adc < 1000)
    {
      result = uint32_t(lx_conv_high * reference_voltage_internal * raw_adc * lux.FACTOR);
      range = 2;
    } else {
      // Read the value of Analog input 2 against VCC for more range
      analogReference(DEFAULT);
      raw_adc = analogRead(lux.ADC_PIN);
      result = uint32_t(lx_conv_high * vcc * raw_adc * lux.FACTOR);
      range = 3;
    }
  }

  // Set the Reference Resistor to 100K to draw the least current and
  // allow more than enough time for the capacitor to charge
  pinMode(lux.CONTROL_PIN, INPUT);
  if (DEBUG)
  {
    // Indent the second reading to make it easier to read the output
    if (idx)
      Serial.print(F("     "));
    Serial.print(F("Lux"));
    Serial.print(idx + 1);
